...
class Discount(models.Model):
    ...
    type = models.CharField(
        _("type"), max_length=8, choices=DISCOUNT_TYPES, default="percent"
    )
...
class SeatMap(models.Model):
    ...
    img = models.ImageField(_("map image"), upload_to="maps", blank=True, null=True)
...
class Gate(models.Model):
    event = models.ForeignKey(
        Event, related_name="gates", verbose_name=_("event"), on_delete=models.CASCADE
    )
...
class SeatLayout(models.Model):
    ...
    direction = models.CharField(
        _("direction"), max_length=2, choices=DIRECTIONS, default="d"
    )
    layout = models.TextField(
        _("seats layout"),
        help_text=_(
            "the layout to select the "
            "numbered seat. "
    ...
    gate = models.ForeignKey(
        Gate, blank=True, null=True, verbose_name=_("gate"), on_delete=models.CASCADE
    )
...
class Space(models.Model):
    event = models.ForeignKey(
        Event, related_name="spaces", verbose_name=_("event"), on_delete=models.CASCADE
    )
    name = models.CharField(_("name"), max_length=300)
    slug = models.SlugField()
...
class ExtraSession(models.Model):
    orig = models.ForeignKey(
        Session,
        related_name="orig_sessions",
        verbose_name=_("orig"),
        on_delete=models.CASCADE,
    )
    extra = models.ForeignKey(
        Session,
        related_name="extra_sessions",
...
class ConfirmEmail(models.Model):
    event = models.OneToOneField(
        Event, related_name="email", verbose_name=_("event"), on_delete=models.CASCADE
    )
...
class InvCode(models.Model):
    event = models.ForeignKey(
        Event, related_name="codes", verbose_name=_("event"), on_delete=models.CASCADE
    )
    ...
    used = models.BooleanField(_("used"), default=False)
    type = models.CharField(
        _("type"), choices=INV_TYPES, default="invited", max_length=15
    )
...
class TicketField(models.Model):
    event = models.ForeignKey(
        Event, related_name="fields", verbose_name=_("event"), on_delete=models.CASCADE
    )
    order = models.IntegerField(_("order"), default=0)
    type = models.CharField(
        _("type"), max_length=100, choices=FIELD_TYPES, default="text"
    )
    ...
    show_in_tws = models.BooleanField(
        _("show in ticket window sale"),
        default=False,
...
def unchecked(sender, instance, created, raw, using, update_fields, **kwargs):
    if instance.show_in_tws:
        for tf in TicketField.objects.filter(
            event=instance.event, show_in_tws=True
...
class TicketTemplate(models.Model):
    ...
    sponsors = models.ImageField(
        _("sponsors"), upload_to="templsponsors", blank=True, null=True
    )
    ...
    contributors = models.ImageField(
        _("footer"), upload_to="templcontributors", blank=True, null=True
    )

    pagesize_width = models.FloatField(
        _("pagesize width"), help_text=_("in cm"), default=15.92
    )
    pagesize_height = models.FloatField(
        _("pagesize height"), help_text=_("in cm"), default=24.62
    )
    left_margin = models.FloatField(
        _("left margin"), help_text=_("in cm"), default=2.54
    )
    right_margin = models.FloatField(
        _("right margin"), help_text=_("in cm"), default=2.54
    )
    bottom_margin = models.FloatField(
        _("bottom margin"), help_text=_("in cm"), default=2.54
    )
