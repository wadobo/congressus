...
class MultiPurchaseView(TemplateView):
    ...
    def post(self, request, ev=None):
        ...
        form = MPRegisterForm(
            request.POST, event=ev, ids=ids, seats=seats, client=client
        )
...
class Register(CreateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        session = get_object_or_404(
            Session, slug=se, space__slug=sp, space__event__slug=ev
        )
        ctx["session"] = session
        return ctx
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if not ctx["error"]:
            ...
            messages.info(
                self.request,
                _(
                    "You should complete the proccess"
...
class ConfirmPaypal(View):
    def post(self, request):
        ...
        if (
            float(tr.amount.total) == tk.get_price()
            and p.intent == "sale"
...
class EmailConfirmPreview(UserPassesTestMixin, View):
    ...
    def get(self, request, id):
        ...
        if event:
            ...
            subject = Template(event.email.subject).render(
                Context({"ticket": ticket, "extra": extra})
            )
            body = Template(event.email.body).render(
                Context({"ticket": ticket, "extra": extra})
            )
            sname = _("SUBJECT")
            bname = _("BODY")
