...
def search_seats(session, amount):
    ...
    if session.autoseat_mode == "ASC":
        ...
        for layout in autoseats.split(","):
            _layout = session.space.seat_map.layouts.filter(name=layout.strip()).first()
            if _layout:
    ...
    for layout in layouts:
        ...
        avail = layout.contiguous_seats(
            amount, hold_seats, layout.column_start_number, row_rand=row_rand
        )
        if not avail:
            continue
        best_avail = {
