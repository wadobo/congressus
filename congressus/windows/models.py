...
class TicketWindow(models.Model):
    event = models.ForeignKey(
        Event, related_name="windows", verbose_name=_("event"), on_delete=models.CASCADE
    )
    ...
    code = models.CharField(
        _("code"), max_length=5, help_text=_("code to show in tickets")
    )
    ...
    singlerow_pos = models.CharField(
        _("single row position"), default="R", choices=SINGLEROW_POS, max_length=1
    )

    supplement = models.FloatField(
        _("increments or decrements the ticket price for this window"), default=0
    )

    print_close_timeout = models.FloatField(
        _("PDF ticket window close timeout"),
        help_text=_(
            "Time to auto close the ticket PDF popup window, use 0 to disable auto close"
        ),
        default=1.0,
    )
    shortcuts = models.JSONField(default=dict({"add": 145, "sub": 19, "onoff": 42}))
    autocall_singlerow = models.BooleanField(_("autocall singlerow"), default=True)
    number_of_calls = models.PositiveSmallIntegerField(
        _("number of call"),
...
class TicketWindowSale(models.Model):
    ...
    payment = models.CharField(
        _("payment"), max_length=10, choices=PAYMENT_TYPES, default="cash"
    )
...
class TicketWindowCashMovement(models.Model):
    ...
    type = models.CharField(
        _("type"), max_length=10, choices=CASH_MOVEMENT_TYPES, default="change"
    )
