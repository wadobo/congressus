...
from dashboard.models import CHARTS, Dashboard
from access.models import AC_TYPES, AccessControl, LogAccessControl
from events.models import Event, Session, Space
from tickets.models import MultiPurchase
from windows.models import PAYMENT_TYPES, TicketWindow, TicketWindowSale
...
class GeneralView(TemplateView):
    ...
    DATA_LINE = {"labels": [], "datasets": [deepcopy(MAIN_DATASET)]}
    INDEX_MAIN = 0
    DEFAULT_PIE_DATASET = {
        "backgroundColor": [],
    ...
    def get_random_color(self, extra_alpha=False):
        ...
        if extra_alpha:
            return "rgba(%s,1)" % (",".join(map(str, rand))), "rgba(%s,0.2)" % (
                ",".join(map(str, rand))
            )
        else:
    ...
    def get_sales_online(self, timestep, max):
        ...
        # Fill sales online
        mps = MultiPurchase.objects.filter(
            created__gt=min_date, confirmed=True
        ).order_by("created")
        for mp in mps:
            ...
            for ticket in mp.all_tickets():
                try:
                    ...
                    continue
                extra_index = self.dataset_index(
                    res["datasets"],
                    ticket.session.space.name + "  " + ticket.session.name,
                )
                res.get("datasets")[self.INDEX_MAIN].get("data")[index] += 1
                res.get("datasets")[extra_index].get("data")[index] += 1
        return res
    ...
    def get_bar(self, type, timestep, max):
        ...
        # Create labels and dataset
        datas = (
            TicketWindow.objects.values_list("slug", "cash")
            .filter(event=self.ev)
            .order_by("slug")
        )
    ...
    def get_chart(
        self, type_chart, timestep=settings.TIMESTEP_CHART, max=settings.MAX_STEP_CHART
    ):
        title = ""
        for k, v in CHARTS:
            if k == type_chart:
        ...
        return {"data": chart, "type_data": tdata, "title": title, "type_chart": tchart}
    ...
    def post(self, request, ev, dash):
        ...
        for chart in dashboard.charts.all():
            ctx["charts"].append(
                self.get_chart(chart.type, chart.timestep, chart.max_steps)
            )
...
class ReportView(TemplateView):
    ...
    def get_sessions(self, **filters):
        ...
        for space in self.spaces.all():
            sessions += space.sessions.filter(**filters)
        return sorted(sessions, key=lambda x: x.start)

    def get_days(self):
        return sorted(list(set([session.start.date() for session in self.sessions])))

    def get_context_data(self, *args, **kwargs):
        ctx = super(ReportView, self).get_context_data(*args, **kwargs)
        ev = self.kwargs["ev"]
        self.ev = get_object_or_404(
            Event.objects.prefetch_related(
                Prefetch(
                    "windows", TicketWindow.objects.select_related("event", "user")
                ),
                Prefetch("spaces", Space.objects.select_related("event", "seat_map")),
                Prefetch(
                    "spaces__sessions",
                    Session.objects.select_related(
                        "template", "window_template"
                    ).prefetch_related("extra_sessions"),
                ),
            ),
            slug=ev,
        )
...
class GeneralReportView(ReportView):
    ...
    def get_context_data(self, *args, **kwargs):
        ctx = super().get_context_data(*args, **kwargs)
        all_windows = TicketWindow.objects.select_related("event").filter(event=self.ev)
        ctx["all_windows"] = all_windows
        ctx["online_windows"] = []
        ctx["local_windows"] = []
        for window in all_windows:
            if window.online:
                ctx["online_windows"].append(window)
            else:
                ctx["local_windows"].append(window)
...
class OnlineReportView(ReportView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if start_date and end_date:
            ...
            self.sessions = self.get_sessions(start__range=(start_date, end_date))
        ...
        ctx["session_days"] = [
            (d, self.get_sessions(start__range=(d, d + delta))) for d in days
        ]
        ctx["sessions_grouped"] = self.sessions_grouped(self.sessions)
...
class CountReportView(ReportView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        delta = timedelta(days=1)
        ctx["session_days"] = [
            (d, self.get_sessions(start__range=(d, d + delta))) for d in session_days
        ]
...
class AccessReportView(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        for access in self.access:
            ...
            for num, day in enumerate(days):
                amount_access = access.log_access.filter(
                    date__date=day, status__in=["ok", "right"]
                ).count()
    ...
    def event_days(self) -> list[datetime.date]:
        ...
        return [
            ...
            for days in range(0, (last_date - first_date).days + 1)
        ]
